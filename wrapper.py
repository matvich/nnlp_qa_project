...
def wrap_message_list(m_list, insert_intro=True, wrap_type='QA', check_end_punct=True):
    '''
    Parameters:
    ----------
    m_list : list
...
def output_post_processing(input_quote, max_words):
    ...
    first_Alice = input_quote.find('Q:') 
    if(first_Alice != -1):
        input_quote = input_quote[:first_Alice] # cut the string when 'Alice:' found'

    input_quote = input_quote.replace("A:", '¿') # filter out "Bob: "
...
def main_wrapper():
    ...
    #         input_text = input("Enter your message here: ")
            output_text = produce_answer(question, messages, 30, 10, 1.0, False, model, enc, device, insert_intro=True, wrap_type='QA')
